...
if ( !class_exists('wps_cart') ) {
	...
	class wps_cart {
		...
		function display_cart( $args ) {
			...
			$output  = '<div id="wps_cart_container">';
			$output .= self::cart_content($cart_type);
			$output .= '</div>';
			return $output;
		}
		
		/** Cart Content **/
		function cart_content( $cart_type = '' ) {
			$output = '';
			...
			if ( !empty($cart_content) ) {
				...
				if ( !empty($cart_items) ) {
					...
					return __( 'Your cart is empty', 'wpshop' );
				}
			}
			else {
				return __( 'Your cart is empty', 'wpshop' );
		...
		function display_mini_cart() {
			...
			ob_start();
			require_once( $this->get_template_part( "frontend", "mini-cart/mini-cart") );
			$output = ob_get_contents();
			ob_end_clean();
			return $output;
		}
		...
		function mini_cart_content() {
			...
			if ( !empty($cart_content) ) {
				...
				if ( !empty($cart_items) ) {
					...
					ob_start();
					require( $this->get_template_part( "frontend", "mini-cart/mini-cart", "content") );
					$output = ob_get_contents();
					ob_end_clean();
				}
				else {
					$output = __( 'Your cart is empty', 'wpshop' );
				}
			}
			else {
				$output = __( 'Your cart is empty', 'wpshop' );
		...
		function display_resume_cart() {
			...
			ob_start();
			require_once( $this->get_template_part( "frontend", "resume-cart/resume-cart") );
			$output = ob_get_contents();
			ob_end_clean();
			return $output;
		}
		...
		function resume_cart_content() {
			...
			if ( !empty($cart_content) ) {
				...
				if ( !empty($cart_items) ) {
					...
					$total_ati  = $total_cart = ( !empty($cart_content['order_amount_to_pay_now']) ) ? $cart_content['order_amount_to_pay_now'] : 0;
					ob_start();
					require_once( $this->get_template_part( "frontend", "resume-cart/resume-cart", "content") );
					$output = ob_get_contents();
					ob_end_clean();
				}
				else {
					$resume_cart_body = __( 'Your cart is empty', 'wpshop' );
				}
			}
			else {
				$resume_cart_body = __( 'Your cart is empty', 'wpshop' );
		...
		function display_apply_coupon_interface() {
			...
			if ( !empty( $_SESSION) && !empty($_SESSION['cart']) && !empty($_SESSION['cart']['order_items']) ) {
				ob_start();
				require_once( $this->get_template_part( "frontend", "coupon/apply_coupon") );
				$output = ob_get_contents();
				ob_end_clean();
			}
			return $output;
